...
from ksweb.lib.importers.yaml_importer import YamlImporter
...
class DocumentController(RestController):
    ...
    def put(
        self, _id, title, html, workspace, description, license, version, tags, **kw
    ):
        tags = {__.strip() for __ in tags.split(",")} if tags else []
    ...
    def export(self, _id):
        ...
        response.headerlist.append(
            ("Content-Disposition", str("attachment;filename=%s.yaml" % filename))
        )
        encoded = json_render.encode(document.export())
        json_ = json.loads(encoded)
        return yaml.dump(json_, default_flow_style=False)
    ...
    def import_document(self, workspace, file_import):
        ...
        importer = YamlImporter(file_content, ObjectId(workspace), owner)
        importer.run()
