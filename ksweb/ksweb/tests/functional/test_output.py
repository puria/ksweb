...
class TestOutput(TestController):
    ...
    def test_new_precondition_simple(self):
        ...
        resp_admin = self.app.get(
            "/output/new", params=dict(workspace=self.workspace._id)
        )
        assert resp_admin.status_code == 200
    ...
    def test_creation_output(self):
        ...
        precondition = self._create_fake_simple_precondition(
            "Precondition 1", workspace1._id
        )
        ...
        resp = self.app.post_json("/output/post", params=output_params).json
    ...
    def test_creation_output_with_fake_qa_related(self):
        ...
        output_params = {
            ...
            "precondition": str(precondition._id),
            "content": [
                {"type": "text", "content": "content", "title": ""},
                {
    ...
    def test_put_output(self):
        ...
        self.app.put_json("/output/put", params=output_params).json
    ...
    def test_output_post_without_filter(self):
        ...
        resp = self.app.post_json("/output/post", params=output_params, status=200).json
    ...
    def test_put_output_with_fake_precondition(self):
        ...
        resp = self.app.put_json("/output/put", params=output_params, status=412).json

        assert resp is not None
        assert resp["errors"] is not None, resp
        assert resp["errors"]["precondition"] == "Filter does not exists", resp[
            "errors"
        ]
    ...
    def test_put_output_with_fake_qa_related(self):
        ...
        assert resp["errors"][
            "content"
        ] == "The Question/s {'%s'} is not related to the filter" % str(
            fake_qa._id
        ), resp[
            "errors"
        ][
    ...
    def test_update_output_with_related_entities(self):
        ...
        response = self.app.get(
            "/resolve/original_edit", params=dict(workspace=output_params["workspace"])
        )
        response.follow()
...
class TestOutputPlus(TestController):
    ...
    def test_output_plus_creation(self):
        ...
        result = self.app.get(
            "/output_plus/post",
            params=dict(
                highlighted_text="output_plus", workspace=str(self.workspace._id)
            ),
            status=200,
        )
