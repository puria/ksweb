...
from ksweb.lib.utils import export_qa, export_preconditions, export_outputs, import_qa, to_object_id, upsert_document, \
    import_precondition, import_output
...
class TestUtils(TestController):
    ...
    def test_upsert_document(self):
        inserted = upsert_document(model_class=model.Qa, _owner=ObjectId(self._get_user('lawyer1@ks.axantweb.com')._id),
        ...
        fetched = Qa.query.find({'title': 'title'}).all()
