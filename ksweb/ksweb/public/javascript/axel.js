/*!
 * Loader with collision detection based
 * on headjs. Loads scripts based on a name
 * instead of path avoiding the load of the
 * same script multiple times without the
...
; (function(win, undefined) {
    ...
    api.register = function(name, path) {
        ...
        if(path in registrations_by_path)
            return api.alias(registrations_by_path[path], name);

        var root_name = api.resolve(name);
        if (root_name == null) {
            registrations[name] = [path];
            aliases[name] = null;
            if (path)
                registrations_by_path[path] = name;
        }
        else {
            var current_registration = registrations[root_name];
            if ((current_registration[0] === null) || (root_name != name)) {
                current_registration.push(path);
                if (path)
                    registrations_by_path[path] = root_name;
            }
        }
    ...
    api.alias = function(name, alias) {
        if (name === alias)
            return api;

        if ((name in registrations) && (alias in registrations))
            throw {'type':'AlreadyRegistered', 'message':'Trying to alias two modules which are both ' +
                                                         'already registered, unable to decide the precedence'};
    ...
    api.resolve = function(name) {
        ...
        var visits = [];
        var root_alias = name;
        while (aliases[root_alias] != null) {
            root_alias = aliases[root_alias];
            if (visits.indexOf(root_alias) != -1)
                return null;

            visits.push(root_alias);
        }

        return root_alias;
    };

    api.path = function(name) {
        var original_name = api.resolve(name);
        if (original_name === null)
            return null;

        var paths = registrations[original_name];
    ...
    api.load = function(name, callback) {
        ...
        for (var i=0; i<name.length; ++i) {
            ...
            if (original_name == null)
                throw {'type':'NotFound', 'message':'module not found, please call registration ' +
                                                    'or alias before using load'};

            var original_path = api.path(name[i]);
            if (original_path == null)
                throw {'type':'NotRegistered', 'message':'module has no paths, please call registration ' +
...
; (function (win, undefined) {
    ...
    //If HeadJS is already provided avoid defining it again.
    var headVar = win.head_conf && win.head_conf.head || "head";
    if (win[headVar])
        return;
...
/* 
 Provide compatibility with IE8 missing the indexOf method for arrays
